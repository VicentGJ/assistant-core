...
class SQLAlchemyDatabase(DatabaseInterface):
    def __init__(self):
        try:
            self.engine = create_engine(settings.db_url + "?sslmode=disable", pool_size=10, max_overflow=20)
            self.session_local = sessionmaker(autocommit=False, autoflush=False, bind=self.engine)
            Base.metadata.create_all(bind=self.engine)  # TODO: This is not working properly with Supabase
    ...
    def read(self, model: AbstractModel, filter_by: dict | None = None) -> list[AbstractModel]:
        try:
            db: Session = next(self._get_db())
            if filter_by:
    ...
    def update(self, model: AbstractModel, filter_by: dict, update_values: dict) -> list[AbstractModel]:
        try:
            db: Session = next(self._get_db())
            objs: list[AbstractModel] = db.query(model).filter_by(**filter_by).all()
            for obj in objs:
